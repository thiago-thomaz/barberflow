...
async function main() {
  const sql = `SELECT json_build_object('id', id, 'status', status, 'uuid', deployment_uuid, 'updated_at', updated_at, 'logs', logs) FROM application_deployment_queues WHERE id >= 211 ORDER BY id ASC;`;
  ...
  for (const row of rows) {
    ...
    try {
      ...
      if (Array.isArray(logs)) {
        ...
        console.log(textLines.slice(-30).join('\n'));
      }
    } catch(e) {
